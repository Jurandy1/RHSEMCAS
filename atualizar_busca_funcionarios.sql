CREATE OR REPLACE FUNCTION public.fn_buscar_funcionarios(
  p_termo text DEFAULT NULL::text, 
  p_vinculo_id bigint DEFAULT NULL::bigint, 
  p_lotacao_id bigint DEFAULT NULL::bigint, 
  p_funcao text DEFAULT NULL::text, 
  p_turno_id bigint DEFAULT NULL::bigint, 
  p_limite integer DEFAULT 50, 
  p_offset integer DEFAULT 0, 
  p_order_by text DEFAULT 'nome'::text, 
  p_order_dir text DEFAULT 'asc'::text
)
 RETURNS TABLE(funcionario_id bigint, nome text, vinculo text, funcao text, lotacao_nome text, caminho_lotacao text, turno text, total bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
begin
    ...
    resultados as (
        ...
        where fl.ativo = true
          and f.ativo = true
          -- AQUI ESTÁ A ALTERAÇÃO: Adicionamos a busca pela matrícula (f.matricula)
          and (p_termo is null or p_termo = '' or f.nome ilike '%' || p_termo || '%' or f.matricula ilike '%' || p_termo || '%')
